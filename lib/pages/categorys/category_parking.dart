...
class CategoryParkingPage extends StatefulWidget {
  const CategoryParkingPage({super.key});

  @override
  State<CategoryParkingPage> createState() => _CategoryParkingPageState();
}

class _CategoryParkingPageState extends State<CategoryParkingPage> {
  final CategoryController _categoryController = CategoryController();

  bool _isLoading = true;
  String? _errorMessage;
  List<KakaoPlace> _parkingResults = [];
  ...
  Future<void> _initializeAndSearch() async {
    try {
      ...
      setState(() {
        _errorMessage = '주차장 검색 중 오류가 발생했습니다: $e';
        _isLoading = false;
      });
    }
  }
  ...
  Future<void> _searchNearbyParking() async {
    try {
      ...
      print('주차장 검색 결과: ${results.length}개');

      setState(() {
        _parkingResults = results;
        _isLoading = false;
      });

      if (results.isNotEmpty) {
        print(
          '가장 가까운 주차장: ${results.first.placeName} (${results.first.distanceKm.toStringAsFixed(1)}km)',
        );
      }
    } catch (e) {
      setState(() {
        _errorMessage = '주차장 검색 실패: $e';
        _isLoading = false;
      });
    }
  }
  ...
  Widget _buildBody(double screenWidth, double screenHeight) {
    ...
    return ListView.builder(
      ...
      itemCount: _parkingResults.length,
      itemBuilder: (context, index) {
        final place = _parkingResults[index];
        return _buildParkingCard(place, screenWidth, screenHeight);
      },
    );
  }
