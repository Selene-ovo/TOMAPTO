...
class CategoryGasPage extends StatefulWidget {
  const CategoryGasPage({super.key});

  @override
  State<CategoryGasPage> createState() => _CategoryGasPageState();
}

class _CategoryGasPageState extends State<CategoryGasPage> {
  final CategoryController _categoryController = CategoryController();

  bool _isLoading = true;
  String? _errorMessage;
  List<KakaoPlace> _gasResults = [];
  ...
  Future<void> _initializeAndSearch() async {
    try {
      ...
      if (_userPosition != null) {
        await _searchNearbyGasStations();
      } else {
        setState(() {
      ...
      setState(() {
        _errorMessage = '주유소 검색 중 오류가 발생했습니다: $e';
        _isLoading = false;
      });
    }
  }

  Future<void> _searchNearbyGasStations() async {
    try {
      ...
      print('주유소 검색 결과: ${results.length}개');
      ...
      if (results.isNotEmpty) {
        print(
          '가장 가까운 주유소: ${results.first.placeName} (${results.first.distanceKm.toStringAsFixed(1)}km)',
        );
      }
    } catch (e) {
      setState(() {
        _errorMessage = '주유소 검색 실패: $e';
        _isLoading = false;
      });
    }
  }
  ...
  Widget _buildBody(double screenWidth, double screenHeight) {
    ...
    return RefreshIndicator(
      ...
      child: ListView.builder(
        ...
        itemCount: _gasResults.length,
        itemBuilder: (context, index) {
          final place = _gasResults[index];
          return _buildGasCard(place, screenWidth, screenHeight);
        },
      ),
    );
  }
