...
class _ProfileEditPageState extends State<ProfileEditPage> {
 ...
 Future<void> _refreshImageUrl() async {
   if (_isImageRefreshing) return;
   
   setState(() {
     _isImageRefreshing = true;
   });
   ...
   try {
     ...
     print('이미지 URL 갱신 요청 시작');
     print('API URL: ${_getApiBaseUrl()}/account/profile-edit/refresh-image-url');
     
     final response = await http.post(
       Uri.parse('${_getApiBaseUrl()}/account/profile-edit/refresh-image-url'),
       headers: {
         'Content-Type': 'application/json',
         'Authorization': 'Bearer $token',
       },
     ...
     print('이미지 URL 갱신 응답: ${response.statusCode}');
     ...
     if (response.statusCode == 200) {
       try {
         ...
         if (data['success'] == true && data['data'] != null) {
           ...
           throw Exception(data['message'] ?? 'URL 갱신에 실패했습니다.');
         }
       } catch (jsonError) {
       ...
       throw Exception('API 엔드포인트를 찾을 수 없습니다. 서버 라우팅을 확인해주세요.');
     } else {
       try {
         final data = jsonDecode(response.body);
         throw Exception(data['message'] ?? 'URL 갱신에 실패했습니다.');
     ...
     print('이미지 URL 갱신 오류: $e');
     ScaffoldMessenger.of(context).showSnackBar(
       SnackBar(
         content: Text('이미지 URL 갱신에 실패했습니다: $e'),
         backgroundColor: Colors.red,
       ),
     );
   } finally {
     setState(() {
       _isImageRefreshing = false;
     });
   }
 }
 ...
 Future<void> _uploadProfileImage() async {
   ...
   try {
     ...
     var request = http.MultipartRequest(
       'POST',
       Uri.parse('${_getApiBaseUrl()}/account/profile-edit/upload-profile-image'),
     );
       ...
       children: [
         ...
         Positioned(
           ...
           child: Container(
             ...
             child: _isImageUploading || _isImageRefreshing
                 ? SizedBox(
                     width: 16,
                     height: 16,
                     child: CircularProgressIndicator(
 ...
 void _showImageActionDialog() {
   showDialog(
     ...
     builder: (context) => AlertDialog(
       ...
       content: Text(
         '이미지를 새로 업로드하거나 현재 이미지 URL을 갱신할 수 있습니다.',
         style: TextStyle(fontFamily: 'Pretendard'),
       ),
       actions: [
         TextButton(
   ...
   final result = await Navigator.push(
     ...
     MaterialPageRoute(
       builder: (context) => PasswordResetPage(
         prefilledUserId: widget.currentUserId,
       ),
     ),
   );
